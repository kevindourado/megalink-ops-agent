...
async function callMcpTool(toolName, args) {
  ...
  console.log(`🔧 [MCP] Resposta raw: ${rawText.slice(0, 600)}`);
...
// ─── Rota de diagnóstico: descobre tools disponíveis no api-mcp ───────────────

app.get("/api/debug/tools", async (req, res) => {
  if (!API_MCP_TOKEN) {
    return res.status(500).json({ error: "API_MCP_TOKEN não configurado" });
  }
  try {
